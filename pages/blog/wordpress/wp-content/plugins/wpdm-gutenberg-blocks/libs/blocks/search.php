...
class Search{
    ...
    function output( $attributes, $content){
        ...
        if(method_exists($WPDM->shortCode, 'searchResult')) {
            $attributes['css_class'] = isset($attributes['className'])?$attributes['className']:'wpdm_search_results';
            $attributes['async'] = 1;
            return "<section class='__wpdm_gb_section __wpdm_gb_packages'>".$WPDM->shortCode->searchResult($attributes)."</section>";
        }
        else
            return \WPDM_Messages::info("Block is available with the pro version only!",  -1);
    }
