...
if ( ! class_exists( 'Kreeti_Lite_Info' ) ) {
	...
	class Kreeti_Lite_Info {
		 ...
		 * Single instance.
		 *
		 * @var Kreeti_Lite_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Kreeti_Lite_Info ) ) {
				self::$instance = new Kreeti_Lite_Info;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s info', 'kreeti-lite' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s info', 'kreeti-lite' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-details';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme details page.', 'kreeti-lite' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'kreeti-lite' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'covernews_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'kreeti-lite' ) . '</a></p>';
		}
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'kreeti-lite' ), $method );
			}
		}
		...
		public function useful_plugins() {
			...
			<div class="aft-section aft-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'kreeti-lite' ); ?></a>
				<?php endif; ?>
            ...
            <table class="table">
                <thead>
                    <tr>
                            ...
                            <th class="db-bk-color-one"><?php _e('Product Features', 'kreeti-lite'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Free', 'kreeti-lite'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Premium', 'kreeti-lite'); ?></th>
                    </tr>
                </thead>
                 <tbody>
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'kreeti-lite-info', get_template_directory_uri() . '/lib/info/css/info.css', array(), '1.0.0' );
				wp_enqueue_script( 'kreeti-lite-info', get_template_directory_uri() . '/lib/info/js/info' . $min . '.js', array(), '1.0.0' );
			}
